...
class Image_loader():
    def __init__(self, img_dir: str, downscale_factor: float):
        # loading the Camera intrinsic parameters CameraMatrix
...
class Sfm():
    ...
    def find_features(self, image_0, image_1) -> tuple:
        ...
        return np.float32([key_points_0[m.queryIdx].pt for m in feature]), np.float32([key_points_1[m.trainIdx].pt for m in feature]), key_points_0, key_points_1, good_feature

    def __call__(self, enable_bundle_adjustment: boolean = False):
        cv2.namedWindow('image 1', cv2.WINDOW_NORMAL)
        cv2.namedWindow('image 2', cv2.WINDOW_NORMAL)
        cv2.namedWindow('image match', cv2.WINDOW_NORMAL)
        ...
        feature_0, feature_1, key_point_0, key_point_1, good_feature = self.find_features(image_0, image_1)
        ...
        _, _, feature_1, points_3d, _ = self.PnP(points_3d, feature_1, self.img_obj.CameraMatrix, np.zeros((5, 1), dtype=np.float32), feature_0, initial=1)
        ...
        for i in tqdm(range(total_images - 2)):
            if (i%1) == 0:
                ...
                features_cur, features_2, key_point_0, key_point_1, good_feature = self.find_features(image_1, image_2)
                # keypoint0 = cv2.drawKeypoints(image_1, key_point_0, None, color=(0, 255, 0))
                # keypoint1 = cv2.drawKeypoints(image_2, key_point_1, None, color=(0, 255, 0))
                # cv2.imshow('image 1', keypoint0)
                # cv2.imshow('image 2', keypoint1)
                # cv2.waitKey()
                # matchfeature = cv2.drawMatchesKnn(image_1, key_point_0, image_2, key_point_1, good_feature, None,flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
                # cv2.imshow('image match', matchfeature)
                if i != 0:
                    feature_0, feature_1, points_3d = self.triangulation(pose_0, pose_1, feature_0, feature_1)
...
if __name__ == '__main__':
    sfm = Sfm("Datasets\FileAnh\Chairblack")
    sfm()
